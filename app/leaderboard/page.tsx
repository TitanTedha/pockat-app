    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 md:px-8 mt-6 md:mt-10 mb-24 space-y-6 md:space-y-8">
      ...
      <div className="space-y-4">
        {rankedUsers.map((user, index) => {
          ...
          let rankMedal = <span className="font-black text-amber-900/40 text-base md:text-lg">#{rank}</span>;
          let cardStyle = "bg-[#FFFDF7] border-amber-100 hover:border-amber-300";
          
          if (rank === 1) {
            rankMedal = <span className="text-2xl md:text-3xl">👑</span>;
          ...
          if (isCurrentUser) {
            cardStyle += " ring-2 ring-amber-500 ring-offset-2"; 
          }

          return (
            // Decreased padding on mobile (p-3 vs p-6) and added gap-2 to prevent inner elements from touching
            <div key={user.id} className={`flex items-center justify-between p-3 md:p-6 rounded-3xl border-2 transition-all gap-2 ${cardStyle}`}>
              
              {/* THE FIX: Added min-w-0 and flex-1 so the left container knows it is allowed to shrink */}
              <div className="flex items-center gap-3 md:gap-6 min-w-0 flex-1">
                <div className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center shrink-0">
                  {rankMedal}
                </div>
                
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden bg-amber-100 flex items-center justify-center border-2 border-white shadow-sm shrink-0">
                  {user.image ? (
                ...
                {/* THE FIX: Added min-w-0 here to tell the text it can be cut off */}
                <div className="min-w-0 flex-1">
                  <h3 className="font-extrabold text-amber-950 text-sm md:text-lg flex items-center gap-2 min-w-0">
                    {/* THE FIX: Added 'truncate' so long names get "..." instead of pushing the UI */}
                    <span className="truncate">{user.name}</span>
                    {isCurrentUser && <span className="bg-amber-500 text-white text-[9px] md:text-[10px] px-2 py-0.5 rounded-full uppercase tracking-wider shrink-0">You</span>}
                  </h3>
                  <p className="text-[10px] md:text-xs font-bold text-amber-800/50 mt-0.5 truncate">
                    Saved {formatCurrency(displaySaved)}
                  </p>
                </div>
              </div>

              <div className="text-right shrink-0 ml-1 md:ml-2">
                <div className="bg-white border border-amber-200 px-3 py-1.5 md:px-4 md:py-2 rounded-2xl shadow-inner inline-block text-center">
                  <span className="text-base md:text-xl font-black text-green-600 block leading-none">{displayRate}%</span>
                  <span className="text-[8px] md:text-[10px] font-bold text-amber-800/40 block mt-1 uppercase tracking-wide leading-none">Rate</span>
                </div>
              </div>
